...
export async function getNamesOfImages (idOfSelectField) {
  ...
  let arrayOfNames = []
  // console.log(idOfSelectField)
  const folder = idOfSelectField === 1 ? 'tractorImages' : idOfSelectField === 2 ? 'attachedMachinery' : null
  // console.log(folder)
...
export async function getImage(id, value, allNamesOfImages, nameOfElement) {
   return new Promise((resolve, reject)=>{
    ...
    if (id === 1 || id === 2) {
     ...
     storage.refFromURL(config + folderArray[id - 1] + filterImagesToExactElement[0])
